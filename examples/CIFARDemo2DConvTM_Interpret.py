...
for X_batch, Y_batch in datagen.flow(X_train, Y_train, batch_size=10000):
        ...
        if config.interpret:
            ...
            if max > 70.0:
                ...
                Incorrect_class_instances = []
                Incorrect_class_instances_original = []
                for q in range(X_test.shape[0]):
                    if Y_test[q] == 8 and tm.predict(X_test[q]) == Y_test[q]:
                        Max_class_instances.append(X_test[q])
                        Max_class_instances_original.append(X_test1[q])
                        break
                
                outputs = np.zeros((32,32)).astype(np.uint8)
                ...
                for p in range(100):
                    output = np.zeros((32,32)).astype(np.uint8)
